...
public class Fly
{
    ...
    */
    ...
    {
        int row = world.getFlyLocation().getRow();
        int column = world.getFlyLocation().getCol();
        if (direction == FlyWorldGUI.NORTH && world.isValidLoc(row - 1,column) == true){
            world.getLocation(row-1, column).setFly(this);
            world.getLocation(row, column).removeFly();
            location = world.getLocation(row-1, column);
        }
        else if (direction == FlyWorldGUI.SOUTH && world.isValidLoc(row + 1,column) == true){
            world.getLocation(row+1, column).setFly(this);
            world.getLocation(row, column).removeFly();
            location = world.getLocation(row+1, column);       }
        else if (direction == FlyWorldGUI.EAST && world.isValidLoc(row,column+1) == true){
            world.getLocation(row, column+1).setFly(this);
            world.getLocation(row, column).removeFly();
            location = world.getLocation(row, column+1); }
        else if (direction == FlyWorldGUI.WEST && world.isValidLoc(row,column-1) == true){
            world.getLocation(row, column-1).setFly(this);
            world.getLocation(row, column).removeFly();
            location = world.getLocation(row, column-1);   }
        }
